...
if(a.equals("insert")){
    ...
    String charuid = new CommDAO().insert(request,"tuipiao",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
new CommDAO().commOper("update goupiao set zhuangtai='已退票' where id='"+request.getParameter("goupiaoid")+"'");

new CommDAO().commOper("update changci set shengyupiaoshu=shengyupiaoshu+'"+request.getParameter("tuipiaozhangshu")+"' where bianhao='"+request.getParameter("bianhao")+"'");
    ...
    String charuid = new CommDAO().update(request,"tuipiao",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("tuipiao").find(Request.get("id"));
    
    new CommDAO().delete(request,"tuipiao");  // 执行删除命令
