...
if(a.equals("insert")){
    ...
    String charuid = new CommDAO().insert(request,"goupiao",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
new CommDAO().commOper("UPDATE goupiao SET xiaoji=(goumaipiaoshu*jiage) WHERE id='"+charuid+"'");
    ...
    String charuid = new CommDAO().update(request,"goupiao",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法

    new CommDAO().commOper("UPDATE goupiao SET xiaoji=(goumaipiaoshu*jiage) WHERE id='"+request.getParameter("id")+"'");
    ...
    // 获取一行数据
    HashMap map = Query.make("goupiao").find(Request.get("id"));
    
    new CommDAO().delete(request,"goupiao");  // 执行删除命令
