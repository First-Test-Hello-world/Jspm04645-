...
if(a.equals("insert")){
    ...
    String charuid = new CommDAO().insert(request,"pingjia",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
new CommDAO().commOper("update goupiao set zhuangtai='已结束' where id='"+request.getParameter("goupiaoid")+"'");
    ...
    String charuid = new CommDAO().update(request,"pingjia",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("pingjia").find(Request.get("id"));
    
    new CommDAO().delete(request,"pingjia");  // 执行删除命令
