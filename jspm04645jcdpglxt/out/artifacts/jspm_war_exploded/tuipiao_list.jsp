...
where+=" AND yanyichangsuo ='"+request.getParameter("yanyichangsuo")+"'"; } 
 ...
 if(request.getParameter("goumaipiaoshu_end") != null && !"".equals( request.getParameter("goumaipiaoshu_end") )){ where+= " AND goumaipiaoshu <='"+request.getParameter("goumaipiaoshu_end")+"' "; }                    List<HashMap> list = Query.make("tuipiao").where(where).order(orderby+" "+sort).page(15);
...
<div class="panel panel-default">
    ...
    演艺场所
                            ...
                            <select class="form-control class_yanyichangsuo17" data-value="<%= request.getParameter("yanyichangsuo") !=null ? request.getParameter("yanyichangsuo") : "" %>" id="yanyichangsuo" name="yanyichangsuo">
...
<script>
$(".class_yanyichangsuo17").val($(".class_yanyichangsuo17").attr("data-value"))</script>
...
<td> <% 
 HashMap mapyanyiting22 = new CommDAO().find("SELECT mingcheng FROM yanyiting where id='"+map.get("yanyichangsuo")+"'"); 
 %><%= mapyanyiting22.get("mingcheng") %> </td>
<td> <%= map.get("kaichangshijian") %> </td>
<td> <%= map.get("jieshushijian") %> </td>
<td> <%= map.get("jiage") %> </td>
<td> <%= map.get("goumaipiaoshu") %> </td>
<td> <%= map.get("xiaoji") %> </td>
<td> <%= map.get("tuipiaoyuanyin") %> </td>
<td> <%= map.get("tuipiaozhangshu") %> </td>
<td> <%= map.get("tuipiaoren") %> </td>
                                                                        <td align="center"><%= map.get("addtime") %></td>
                        <td align="center">
                            
                                                                                    <a href="tuipiao_updt.jsp?id=<%= map.get("id") %>">修改</a>
                            <a href="tuipiao.jsp?a=delete&id=<%= map.get("id") %>" onclick="return confirm('真的要删除？')">删除</a>
                            <!--qiatnalijne-->
                        </td>
                    </tr>
                    <% } %>
